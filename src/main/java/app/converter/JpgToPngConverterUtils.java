...
public class JpgToPngConverterUtils extends ConverterUtils implements FileConverter {

    private static final Logger LOGGER = LoggerFactory.getLogger(JpgToPngConverterUtils.class);
    ...
    public void convert(File source, File dest, Map<String, Object> options) throws ConversionErrorException {
        LOGGER.info("Avvio conversione da JPG a PNG: {} -> {}", source.getAbsolutePath(), dest.getAbsolutePath());
        if (!source.exists()) {
        ...
        try {
            ...
            if (grabber.getLengthInFrames() > 1) {
                LOGGER.error("Il file JPG è animato e non è supportato: {}", source.getAbsolutePath());
                throw new ConversionErrorException("Il file JPG è animato e non è supportato: " + source.getAbsolutePath());
            }
